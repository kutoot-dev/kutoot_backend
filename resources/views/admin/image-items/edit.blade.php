  ...
  <section class="section">
    <div class="section-header">
      ...
      <div class="section-header-breadcrumb">
        ...
        <div class="breadcrumb-item">{{__('Edit Image')}}</div>
      </div>
    </div>
    ...
    <div class="section-body">
      <div class="row">
        <div class="col-12">
          <div class="card">
            <form action="{{ route('admin.image-items.update', $imageItem->id) }}" method="POST" enctype="multipart/form-data">
              @csrf
              @method('PUT')
              <div class="card-body">
                <div class="form-group">
                  <label for="title">{{__('Title')}}<span class="text-danger">*</span></label>
                  <input type="text" name="title" id="title" class="form-control" value="{{ old('title', $imageItem->title) }}" required>
                </div>

                <div class="form-group">
                  <label for="description">{{__('Description')}}</label>
                  <textarea name="description" id="description" class="form-control" rows="4">{{ old('description', $imageItem->description) }}</textarea>
                </div>
                ...
                <div class="form-group">
                  ...
                  <select name="image_type_id" id="image_type_id" class="form-control">
                    ...
                    @foreach ($types as $type)
                      <option value="{{ $type->id }}" {{ (old('image_type_id', $imageItem->image_type_id) == $type->id) ? 'selected' : '' }}>
                        {{ $type->name }}
                      </option>
                    @endforeach
                  </select>
                ...
                <div class="form-group">
                  ...
                  <img src="{{ asset('storage/' . $imageItem->image_path) }}" alt="Current Image" style="max-height: 120px;">
                </div>
