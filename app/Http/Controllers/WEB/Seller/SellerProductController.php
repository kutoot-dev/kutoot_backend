...
class SellerProductController extends Controller
{
    ...
    public function getChildcategoryBySubCategory($id){
        $childCategories=ChildCategory::where('sub_category_id',$id)->where('status',1)->get();
    ...
    public function product_import(Request $request)
    {
        ...
        try {
    Excel::import(new ProductImport, $request->file('import_file'));
    return back()->with(['messege'=>'Uploaded Successfully','alert-type'=>'success']);
} catch (\Maatwebsite\Excel\Validators\ValidationException $e) {
    dd($e->failures()); // shows validation errors per row
} catch (\Throwable $ex) {
    dd($ex->getMessage()); // catch any error
}
