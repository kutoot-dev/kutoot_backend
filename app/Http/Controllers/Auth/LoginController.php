...
class LoginController extends Controller
{
    ...
    public function sociallogin(Request $request)
    {
        ...
        try {
            \Log::info('Social login attempt started.');
            ...
            if ($decoded->iss !== "https://securetoken.google.com/{$projectId}") {
                \Log::error('Invalid token issuer', ['iss' => $decoded->iss]);
                throw new \Exception('Invalid token issuer');
            }

            if ($decoded->aud !== $projectId) {
                \Log::error('Invalid token audience', ['aud' => $decoded->aud]);
            ...
            \Log::info('Firebase token decoded successfully', ['uid' => $uid, 'provider' => $provider]);
            ...
            \Log::info('User logged in successfully via social login', ['user_id' => $user->id]);
